...
void display(void)
{
    ...
    if (bd != nullptr)
    {
        ...
        std::cout << currPlayerName << "it is your turn" << std::endl;
        if (!prop){ //checks null
            // execute this code if the space is not a property
            displayMsg(bd->getCurrentPlayer()->getMoney(), 0, currSpaceName, currPlayerName);
        } else {
            int currSpacePrice = prop->getCost();
            displayMsg(bd->getCurrentPlayer()->getMoney(), currSpacePrice, currSpaceName, currPlayerName);
        }
        std::cout << "displaying pieces" << std::endl;
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    case 'h':
    case 'H':
        // send message that you want to upgrade (buy house/hotel)
    ...
    case 'R':
        ...
        // board object should be updated here before being redrawn
        /* adding code for random number generator*/
		outMsg.moveType = 'r';
		outMsg.playerRoll = dice_distro(generator); // call RNG engine on 1 to 12 distribution
		cli->sendToServer(outMsg);
